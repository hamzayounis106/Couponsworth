  ...
  const handleLetterClick = (letter) => {
    ...
    const filtered =
      letter === "all"
        ? stores
        : stores.filter(
            (store) => store.name && store.name[0].toUpperCase() === letter
          );
      ...
      <div className="shop-page-alphabet-filter">
        {alphabet.map((letter) => (
          <button
            ...
            className={`shop-page-filter-button ${
              selectedLetter === letter ? "shop-page-active" : ""
            }`}
          >
            {letter}
          </button>
        ...
        <button
          ...
          className={`shop-page-filter-button ${
            selectedLetter === "all" ? "shop-page-active" : ""
          }`}
        >
          All
        </button>
            ...
            <div key={letter} className="shop-page-store-group">
              ...
              <div className="shop-page-stores-block">
                {groupedStores[letter]
                  .slice(0, visibleStores[letter] ? undefined : MAX_VISIBLE)
                  .map((store) => (
                    <div
                      key={store._id}
